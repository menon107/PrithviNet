    ...
    <div style={{
      ...
      {items.map(({ key, label, range }) => (
        <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 11, color: '#9ca3af', fontFamily: "'Space Mono',monospace" }}>{label}</span>
          <span style={{ fontSize: 10, color: '#4b5563', marginLeft: 'auto', paddingLeft: 8 }}>{range}</span>
        </div>
      ))}
    </div>
  );
