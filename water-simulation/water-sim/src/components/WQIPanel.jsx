...
import { PARAMS, computeWQI, wqiToColor, wqiGrade, defaultParams, PRESETS } from '../simulation/wqi.js'
...
const s = {
  ...
  paramRow: {
    display: 'grid', gridTemplateColumns: '82px 1fr 54px',
  ...
  presetBtn: (v) => ({
    ...
    background: v === 'pristine' ? '#22d3ee22' : v === 'urban' ? '#1a2030'
              : v === 'industrial' ? '#fb923c22' : '#ef444422',
    color: v === 'pristine' ? '#22d3ee' : v === 'urban' ? '#9ca3af'
         : v === 'industrial' ? '#fb923c' : '#ef4444',
    transition: 'opacity .15s',
  }),
  applyBtn: {
    width: '100%', padding: '9px 12px', borderRadius: 8, border: 'none', cursor: 'pointer',
    background: '#00d4ff', color: '#0a0e14', fontSize: 12,
    fontFamily: "'Space Mono',monospace", fontWeight: 700, marginTop: 10,
  },
}
...
export default function WQIPanel({ layer, onApply }) {
  ...
  // Sync sliders when the selected layer changes
  useEffect(() => {
    setParams(layer.params ?? defaultParams())
  ...
  return (
    <div>
      {/* WQI score circle + grade */}
      <div style={{
        background: '#111620', borderRadius: 10, padding: '12px 14px',
        ...
        <div style={{
          ...
          justifyContent: 'center', transition: 'border-color .4s',
        }}>
          <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 16, fontWeight: 700, color: '#e8eaf0' }}>
            {wqi}
          </div>
          <div style={{ fontSize: 8, color: '#6b7280', fontFamily: "'Space Mono',monospace", textTransform: 'uppercase', letterSpacing: 1 }}>
            WQI
          </div>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* Per-parameter breakdown bars */}
          {Object.keys(PARAMS).map(key => {
            const { si } = breakdown[key]
            const bc = si >= 80 ? '#22c55e' : si >= 60 ? '#a3e635'
                     : si >= 40 ? '#eab308' : si >= 20 ? '#f97316' : '#ef4444'
            return (
              <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 3 }}>
                <span style={{ fontSize: 9, color: '#6b7280', width: 60, flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {PARAMS[key].label}
                </span>
                ...
                <span style={{ fontSize: 9, fontFamily: "'Space Mono',monospace", color: bc, width: 22, textAlign: 'right' }}>
                  {si}
                </span>
              </div>
            )
      ...
      {/* Apply button */}
      <button style={s.applyBtn} onClick={() => onApply(params)}>
        Apply to Map
      </button>
    </div>
  )
