 ...
 *
 * Weighted sub-index model — each parameter is normalised to 0–100,
 * multiplied by its weight, summed → WQI 0–100.
 * Higher WQI = cleaner water.
 */
...
/** Sub-index for each parameter: returns 0–100 (100 = best quality). */
...
/** Compute WQI and per-parameter breakdown from a params object. */
...
/** Map WQI score to a hex color. */
export function wqiToColor(wqi) {
  if (wqi >= 80) return '#22d3ee'  // Pristine
  if (wqi >= 60) return '#a3e635'  // Good
  if (wqi >= 40) return '#facc15'  // Fair
  if (wqi >= 20) return '#fb923c'  // Poor
...
/** Default params representing clean, natural water. */
